...
import { services as allServices, vehicles as initialVehicles } from "@/lib/data";
...
export function Workflow() {
  ...
  const { user } = useAuth();
  const { addTransactionToQueue } = useOfflineSync();
              ...
              <Form {...vehicleForm}>
                <form onSubmit={vehicleForm.handleSubmit(handleVehicleSubmit)} className="space-y-4">
                  ...
                  <Button type="submit" disabled={isScanning}>
                    {isScanning ? 'Scanning...' : 'Find Vehicle'}
